...
Input_Bulk=Input_Bulk[sortperm(Input_Bulk[:, end-1]), :] # send the loading fault to the end
BulkFaultcount = length(Input_Bulk[:,1])
TotalElemCount =  length(Input_Bulk[:,1])
...
Fault_V_i = Input_Bulk[:,15]
Fault_Friction_i = Input_Bulk[:,16]
Fault_NormalStress = Input_Bulk[:,17] - Input_Bulk[:,18] .*  FaultCenter[:,3]
Fault_V_Const = Input_Bulk[:,19]
Fault_BulkIndex = Rake * 0
FaultLengthStrike = maximum.(eachrow(abs.([P1-P2 P2-P3 P1-P3])))
FaultLengthDip = FaultLengthStrike
FaultStrikeAngle = zeros(TotalElemCount)
FaultDipAngle = zeros(TotalElemCount)
FaultRakeAngle = Rake
FaultLengthStrike_Bulk = FaultLengthStrike
FaultLengthDip_Bulk = FaultLengthStrike
FaultCount = TotalElemCount
...
UnitVector_DipSlip = zeros(TotalElemCount,3)
...
for ElemIdx = 1:TotalElemCount
    ...
    # ### Dipslip Vector Plot
    # ax.quiver(FaultCenter[ElemIdx,1], FaultCenter[ElemIdx,2], FaultCenter[ElemIdx,3], 
    #     UnitVector_DipSlip[ElemIdx,1] * ArrowLength, UnitVector_DipSlip[ElemIdx,2] * ArrowLength,
    #     UnitVector_DipSlip[ElemIdx,3] * ArrowLength,
    ...
    ax.quiver(FaultCenter[ElemIdx,1], FaultCenter[ElemIdx,2], FaultCenter[ElemIdx,3], 
        UnitVector_Slip[ElemIdx,1] * ArrowLength, UnitVector_Slip[ElemIdx,2] * ArrowLength,
        UnitVector_Slip[ElemIdx,3] * ArrowLength,
...
# scatter3D(FaultCenter[:,1],FaultCenter[:,2],FaultCenter[:,3])
...
# println(StiffnessMatrix_Shear)
# StiffnessMatrix_Shear = StiffnessMatrix_Shear1
save(OutputFileName, 
