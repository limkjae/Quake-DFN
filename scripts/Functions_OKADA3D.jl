...
for iii=1:N_CELL
  if c2[iii] == 0
    ...
    TT[iii] = c1[iii] .* F0 .+ c2[iii] .* atan.(XI[iii] .* ET[iii] ./ (Q[iii] .* R[iii]))
  end
end
...
##### Added if c5,c4==0 due to occational NaN ######### 
ALX = zeros(N_CELL)
X11 = zeros(N_CELL)
X32 = zeros(N_CELL)
for i = 1:N_CELL
  if c5[i] == 0
    ...
    if c4[i] == 0
      ALX[i] = log(RXI[i]).*c5[i];
      X11[i] = (F1./(R[i].*RXI[i])).*c5[i];
      X32[i] = ((R[i]+RXI[i]).*X11[i].*X11[i]./R[i]) .*c5[i];
...
for i = 1:N_CELL
  if c5[i] == 0
    ...
    if c4[i] == 0
      ALE[i] = log(RET[i]).*c5[i];
      Y11[i] = (F1./(R[i].*RET[i])).*c5[i];
      Y32[i] = ((R[i]+RET[i]).*Y11[i].*Y11[i]./R[i]).*c5[i];
      # println("2", Y32,"   ", R[i])
...
ATAN = atan.((ET.*(X+Q.*CD)+X.*(R+X).*SD)./(XI.*(R+X).*CD))
ATAN[isnan.(ATAN)] .= 0.0
