...
###################################################################
############### Build Stiffness Matrix  Stirke Slip ###############
###################################################################

function Function_StiffnessMatrixStrikeSlip(Input_Segment, ShearModulus, PoissonRatio)
    FaultCount=size(Input_Segment,1)
    FaultCenter=Input_Segment[:,1:3]
    FaultLengthStrike=Input_Segment[:,4]
    FaultLengthDip=Input_Segment[:,5]
    ...
    @time begin 
    # Threads.@threads for ReceiverIndex=1:FaultCount;
    # Threads.@spawn for ReceiverIndex=1:FaultCount;
        for ReceiverIndex=1:FaultCount;        
            println(ReceiverIndex)
            # Threads.@spawn for SourceIndex=1:FaultCount;
            for SourceIndex=1:FaultCount;
                    ...
                    StiffnessMatrixNormal[ReceiverIndex,SourceIndex] = - Stress_Receiver[3,3]  # compression is negative
                    StiffnessMatrixShear[ReceiverIndex,SourceIndex] = - ReceiverLLRR * Stress_Receiver[1,3]  # right latteral is negative
...
###################################################################
########### Build Stiffness Matrix  Stirke Slip Vector ############
###################################################################

function Function_StiffnessMatrixStrikeSlip_Vector(Input_Segment, ShearModulus, PoissonRatio)
    FaultCount=size(Input_Segment,1)
    FaultCenter=Input_Segment[:,1:3]
    FaultLengthStrike=Input_Segment[:,4]
    FaultLengthDip=Input_Segment[:,5]
    ...
    # Threads.@spawn for SourceIndex=1:FaultCount;
            for SourceIndex=1:FaultCount;
                println(SourceIndex)
                    ...
                    ReceiverCenter = FaultCenter;
                    ReceiverStrikeAngle = FaultStrikeAngle;
                    ReceiverDipAngle = FaultDipAngle;
                    ReceiverLLRR = FaultLLRR;
                    RelativeStrkieAngle = ReceiverStrikeAngle .- SourceStrikeAngle
...
###################################################################
############# Build Stiffness Matrix Reverse Normal ###############
###################################################################

function Function_StiffnessMatrixReverseNormal(Input_Segment, ShearModulus, PoissonRatio)
    FaultCount=size(Input_Segment,1)
    FaultCenter=Input_Segment[:,1:3]
    FaultLengthStrike=Input_Segment[:,4]
    FaultLengthDip=Input_Segment[:,5]
    ...
    # Threads.@spawn for ReceiverIndex=1:FaultCount;
        for ReceiverIndex=1:FaultCount;        
            ...
            for SourceIndex=1:FaultCount;
                    ...
                    StiffnessMatrixNormal[ReceiverIndex,SourceIndex] = - Stress_Receiver[3,3]  # compression is negative
                    StiffnessMatrixShear[ReceiverIndex,SourceIndex] = OrientationCoeff_Receiver * ReceiverLLRR * Stress_Receiver[2,3]  # normal is negative
...
###################################################################
########## Build Stiffness Matrix  Reverse Normal Vector ##########
###################################################################

function Function_StiffnessMatrixReverseNormal_Vector(Input_Segment, ShearModulus, PoissonRatio)
